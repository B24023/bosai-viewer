...
def color_to_shindo(r, g, b):
    if (r, g, b) == (0, 0, 0) or (r, g, b) == (255, 255, 255): return -3.0 
...
async def get_weather(request: Request):
    async with httpx.AsyncClient(timeout=10.0) as client:
        ...
        for office_data in rain_data_list:
            ...
            warn_cities_lv5, warn_cities_lv3, warn_cities_lv2 = [], [], []
            for area in office_data.get('warning', {}).get('class20Items', []):
                city_name = areas.get('class20s', {}).get(area.get('areaCode'), {}).get('name', '不明')
                for kind in area.get('kinds', []):
                    w_code = str(kind.get('code', '')).zfill(2)
                    status = kind.get('status', '')
                    if status not in ['解除', '発表警報・注意報はなし']:
                        if w_code in ['32', '33'] and city_name not in warn_cities_lv5: warn_cities_lv5.append(city_name)
                        elif w_code == '03' and city_name not in warn_cities_lv3: warn_cities_lv3.append(city_name)
                        elif w_code == '10' and city_name not in warn_cities_lv2: warn_cities_lv2.append(city_name)
            
            if warn_cities_lv5:
                rain_alerts.append({'offices': office_name, 'targetName': "特別警報 対象地域", 'url': "https://www.jma.go.jp/bosai/warning/", 'warnName': "大雨特別警報", 'warnLevel': "5", 'time': time_str, 'publishingOffice': office_name, 'targetCities': " ".join(warn_cities_lv5)})
            if warn_cities_lv3:
                rain_alerts.append({'offices': office_name, 'targetName': "警報 対象地域", 'url': "https://www.jma.go.jp/bosai/warning/", 'warnName': "大雨警報", 'warnLevel': "3", 'time': time_str, 'publishingOffice': office_name, 'targetCities': " ".join(warn_cities_lv3)})
            if warn_cities_lv2:
                rain_alerts.append({'offices': office_name, 'targetName': "注意報 対象地域", 'url': "https://www.jma.go.jp/bosai/warning/", 'warnName': "大雨注意報", 'warnLevel': "2", 'time': time_str, 'publishingOffice': office_name, 'targetCities': " ".join(warn_cities_lv2)})
                
        rain_alerts.sort(key=lambda x: int(x['warnLevel']), reverse=True)
            
    return templates.TemplateResponse(
        request=request, name="weather.html", 
        context={"flood_alerts": flood_alerts, "rain_alerts": rain_alerts, "active_page": "weather"}
    )

@app.get("/earthquake", response_class=HTMLResponse)
async def get_earthquake(request: Request):
    return templates.TemplateResponse(
        request=request, name="earthquake.html", 
        context={"active_page": "earthquake"}
    )

@app.get("/api/realtime")
async def get_realtime_data():
    img_data, img_url = None, ""
    try:
        ...
        with urllib.request.urlopen(req_time, timeout=5, context=ctx) as res:
            latest_time_str = json.loads(res.read().decode('utf-8')).get("latest_time")
            
        if latest_time_str:
            dt = datetime.strptime(latest_time_str, "%Y/%m/%d %H:%M:%S")
            date_str, time_str = dt.strftime("%Y%m%d"), dt.strftime("%Y%m%d%H%M%S")
    ...
    if img_data:
        ...
        for pt in OBSERVATION_POINTS:
            ...
            shindo = color_to_shindo(*img.getpixel((x, y))) if 0 <= x < img.width and 0 <= y < img.height else -3.0
            data.append({"id": pt["id"], "name": pt["name"], "lat": pt["lat"], "lng": pt["lng"], "shindo": shindo})
